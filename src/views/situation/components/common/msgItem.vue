<template>
  <div class="wrap" id="wrap">
    ...
    <div id="box">
      <!-- // 内部滚动框 -->
      <div id="marquee">
        <div class="msg-item" v-for="(item, index) in lists" :key="index">
          <template v-if="item.status == '1'">{{item.name}}在 【{{item.time}}】 恢复了正常通讯</template>
          <template v-else>{{item.name}}在 【{{item.time}}】 断开了与中心通讯</template>
        </div>
      </div>
      <!-- //展示的文字 -->
      <!-- <div id="copy"></div> -->
